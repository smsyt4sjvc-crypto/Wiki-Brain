...
- **★★★★ KILL-SWITCH, NOW (analysis):** $106 cracks on the wholesale side propagate to the retail diesel print within days; Edouard shut-ins tonight would push the pass-through into the kill-switch zone ($5.65 retail leg). Both prior firings spared the crack ("don't punish the bottleneck") — a THIRD firing at $106 with a hurricane in the corridor tests whether that restraint survives a retail spike into the midterm window. Refiners (not in the book) are the obvious beneficiary; the vault's basket holds no pure refiner — noted, not graded.
