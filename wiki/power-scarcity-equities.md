# Power-Scarcity Equities — the three tiers

<!-- TIMELINE:BEGIN — auto-generated by tools/timeline_header.py · do not hand-edit -->
> ## ⏱ TIMELINE — read this BEFORE the entry you came for.
> **15 dated entries · 2026-07-01 → 2026-09-20 · refreshed 2026-09-22 10:20 PDT**
>
> This is the note **from the start, in order**. A new inbound is the NEXT TICK of
> this sequence — read the arc first, then the entry. A March line can be the
> load-bearing one; nothing here is ranked by recency.

**2026-07**
- `2026-07-01` `L38` DATA (observed — 2026-07-01, WebSearch; `raw/pjm-capacity-cooling-2026-07-01.md`)
- `2026-07-12` `L69` 2026-07-12 — GEOTHERMAL: the third leg of clean-firm (Jake's Q: "is this nuclear? LNG?")
- `2026-07-12` `L70` DATA (`raw/2026-07-12-zh-geothermal-fervo-quaise.md`; investor confirms via WebSearch)
- `2026-07-15` `L107` 2026-07-15 — tool: bottleneck-within-the-bottleneck screen
- `2026-07-15` `L116` ⚠️ 2026-07-15 ~8:18pm PT — CORRECTION (bull/bear debate forced this): bottleneck ≠ hedge
- `2026-07-17` `L132` 2026-07-17 ~4:39pm PT — PJM auction: grid scarcity CONFIRMED official (7GW short, 500MW new supply)
**2026-08**
- `2026-08-19` `L160` 2026-08-19 ~10:20am PDT — ⭐⭐⭐⭐⭐ **THE NAMED-COUNTERPARTY MAP THE VAULT DID NOT HAVE: 22 hyperscaler→power contracts wi
- `2026-08-22` `L237` 2026-08-22 ~9:00am PDT — ⭐⭐⭐⭐ **NVDA BOUGHT INTO THE POWER BOTTLENECK ITSELF (CLOVERLEAF), AND PENNSYLVANIA TURNED DAT
**2026-09**
- `2026-09-08` `L289` 2026-09-08 ~10:50pm PDT — **THE SOCAL HEATWAVE IS A DISCONFIRMER OF THE GRID-FRAGILITY FRAME, NOT AN EXAMPLE OF IT: 10
- `2026-09-10` `L306` 2026-09-10 ~8:45am PDT — ⭐⭐⭐⭐⭐⭐ **JAKE'S STANDING ENERGY THESIS, TESTED AGAINST SEMIANALYSIS'S SUPPLY-CHAIN DATA: *"it
- `2026-09-12` `L346` 2026-09-12 ~9:40am PDT — ⭐⭐⭐⭐⭐⭐ **THE D.C. CIRCUIT JUST NARROWED §202(c) — THE EXACT AUTHORITY THIS NOTE HAS ON FILE A
- `2026-09-15` `L368` 2026-09-15 ~9:00am PDT — ⭐⭐⭐⭐⭐ **EPA SIGNS THE GHG REPEAL — THE `:L358` REGISTERED MECHANISM FIRES WITHIN 72 HOURS OF 
- `2026-09-16` `L382` 2026-09-16 ~6:55pm PDT — ⭐⭐⭐⭐⭐ **AEMA LAUNCHES: THE 8/7 CO-LOCATION THESIS GOES INSTITUTIONAL — GOOGLE/NVIDIA/EMERALD 
- `2026-09-19` `L393` 2026-09-19 ~12:35pm PDT (covers Thu-Fri; settled per rule 22) — ⭐⭐⭐⭐⭐ **THE WEDGE WEEK COMPLETES: AEMA (9/16 am) → RAT
- `2026-09-20` `L404` 2026-09-20 ~7:45pm PDT — ⭐⭐⭐⭐⭐ **THE RE-CONCENTRATION READ (settled per rule 22): THE BUILDOUT DOESN'T DIE — IT FALLS 

<!-- TIMELINE:END -->

The leveraged-equity expression of the grid-scarcity thesis. Related: [[power-not-petroleum]],
[[ai-capex-cycle]], [[market-fragility]], [[portfolio-state]].

> Firewall: DATA = measured figures/returns with sources. THESIS = the tiering + entry read.
> Origin: heat-wave/HVAC question 2026-07-01 → traced to the structural power-scarcity trade.

## DATA (observed — 2026-07-01, WebSearch; `raw/pjm-capacity-cooling-2026-07-01.md`)
- **PJM capacity price** at cap ~$329–333/MW-day, record 3rd auction running; cumulative
  ~800%+ (2024/25→2025/26) then +22%. Merchants clearing: TLN, CEG, NRG, VST, PSEG.
- **Cooling names, trailing ~1yr:** FIX ~+270%, AAON ~+56%, VRT +30% Q1 sales / +83% EPS.
- DOE §202(c) emergency orders = forced dispatch + cost recovery, NOT subsidy checks.

## The three tiers (THESIS — interpretation, NOT fact)
1. **Merchant IPPs — high beta to scarcity pricing.** VST, CEG, NRG, TLN, PSEG. Capture the
   record PJM capacity prices directly. CEG/VST/TLN are nuclear-heavy → likely overlap the
   user's existing "nuclear names" sleeve ([[portfolio-state]]).
2. **Regulated utilities — low beta.** ETR, D, AEP, SO (already in [[power-not-petroleum]]).
   Earn on load growth + rate base; do NOT capture price spikes. The defensive expression.
3. **Data-center cooling — the "picks-and-shovels" of AI heat.** VRT (pure-play), AAON, FIX.
   Highest-growth, highest-momentum, most extended.

## Entry read (THESIS — NOT advice; sizing is the user's)
- *(analysis)* This is the *leveraged* lobe the vault was missing — [[power-not-petroleum]] had
  midstream/grid/royalty tolls; merchant IPPs are the price-spike play on the same scarcity.
- *(analysis)* ⚠️ **Everything here has already run hard** (FIX +270%, IPPs multi-baggers off the
  capacity repricing). This is a **watchlist / ladder** problem, not a chase-with-calls problem —
  same discipline as the semis rungs. Calls on strength at highs = buying max IV at the top.
- *(analysis)* Heat-wave calls specifically = NO: dated, expiring (~Jul 3), priced. The weather is
  a coincident signal pointing at the structural theme, not a tradeable catalyst itself.
- *(analysis)* Cleanest optionality on the *structural* story (not weather) = longer-dated calls
  bought into a **pullback** in VST or VRT; into strength, laddered equity beats premium.

## Falsifier
- PJM capacity price rolls back below cap / build response floods supply → the IPP scarcity
  premium compresses. Cooling names de-rate if AI-capex buildout stalls on the demand side
  (see [[ai-capex-cycle]] demand-stall vs supply-stall question).

## 2026-07-12 — GEOTHERMAL: the third leg of clean-firm (Jake's Q: "is this nuclear? LNG?")
### DATA (`raw/2026-07-12-zh-geothermal-fervo-quaise.md`; investor confirms via WebSearch)
- **Fervo (EGS, Cape Station UT): drilling rates +143% (3rd-gen design); Sawtooth 7 = 19,448 ft
  w/ 7,500-ft lateral in 21 days (−70% drill time); Phase I power THIS year, Phase II 400 MW 2028;
  ~$5,500/kW tracking → $3,000/kW target** (vs Vogtle actuals $10-15k/kW).
- **Fervo IPO planned (Motley Fool/Yahoo coverage May 2026).** Public-company hooks confirmed:
  **DVN $10M (2023) + led the $244M Feb-2024 round; LBRT $10M; HP drilling partner.**
- **Quaise (superhot, millimeter-wave): $134M Series B initial close (~$230M total); JERA +
  Idemitsu strategic; Project Obsidian, Newberry Volcano OR; first electrons target 2030.**
- DOE $171.5M EGS funding NOFO (Feb 2026).
### THESIS (interpretation — NOT fact)
- *(taxonomy — the answer to Jake's question)* Geothermal is NEITHER nuclear NOR LNG — it's the
  **third leg of clean-firm**: nuclear's PRODUCT (24/7 carbon-free baseload, no fuel logistics, no
  intermittency) delivered with SHALE'S TOOLKIT (horizontal drilling, multi-stage completion, the
  drilling learning curve). Same prize as SMRs and on-site gas: the data-center clean-firm contract.
- *(the learning-curve asymmetry — why the +143% matters)* Nuclear has a NEGATIVE learning curve
  (Vogtle); drilling has the most documented POSITIVE learning curve in energy (shale 2010s). Fervo's
  +143%/−70% numbers ARE the shale curve replaying in hot rock. $5,500→$3,000/kW target undercuts
  everything clean-firm except existing hydro/nuclear uprates. If the curve holds, EGS wins the
  2028+ tranche of the BtM buildout on cost.
- *(the oil-services second act — ties [[power-not-petroleum]] literally)* DVN/LBRT/HP monetizing
  shale IP into power = the drillers' redeployment. JERA (Japan's biggest LNG buyer) seeding
  superhot = the LNG incumbent hedging its own obsolescence; Japan sits on volcanic superhot
  resource (the InP-map pattern: Japan as the quiet leg, again).
- *(timing honesty)* The AI power gap is NOW (2026-28); Fervo Phase II is 2028, Quaise 2030. 400 MW
  = one mid-size campus — proof, not fleet. Geothermal is a 2028+ answer to a 2026 problem; the
  2026-28 tranche still belongs to gas gensets/turbines ([[buildout-bottleneck-map]] BKR/GEV) and
  grid queue-jumping. Same critique as SMRs, but with a real cost curve behind it.
- *(investable expression, honestly)* Pure-plays private — but the **Fervo IPO = the watch item**
  (nomination-pattern risk: the listing will price the story, not a discount; NVTS rule applies —
  don't chase the pop). Indirect: DVN (stake + expertise), LBRT (stake), HP (rigs), ORA (traditional
  geothermal incumbent, not EGS). And geothermal WATTS need the same transformers/switchgear as
  everything else if grid-tied — the bottleneck map's sellers get paid by this leg too.

## Sources
- `raw/pjm-capacity-cooling-2026-07-01.md` (Utility Dive, Zacks/Yahoo, Investing.com).
- `raw/2026-07-12-zh-geothermal-fervo-quaise.md` (ZH 7/12 + Fervo/Devon PRs, LBRT 8-K, IPO coverage).

### 2026-07-15 — tool: bottleneck-within-the-bottleneck screen
`tools/power_equipment_screen.ipynb` — screens the grid/transformer/switchgear/electrical-equipment
+ power-construction layer (ETN/GEV/VRT/HUBB/POWL/NVT/PWR/MYRG/ROK/EMR…) for "already run vs still
cheap": 52w position + 1yr return vs fwd P/E / EV-EBITDA / FCF yield → gap_score (cheap AND un-run).
IPPs (CEG/VST/TLN) + regulated utils included as reference (the "already priced" and "cheap-but-slow"
benchmarks). Premise (analysis): the IPP scarcity trade is consensus/priced; the equipment layer that
physically connects the GW (transformer + electrician shortage, Beige Book 7/15) is the tighter,
less-crowded constraint. Candidate generator only — cheap != good; second pass = FCF/capex quality read.

### ⚠️ 2026-07-15 ~8:18pm PT — CORRECTION (bull/bear debate forced this): bottleneck ≠ hedge
The 7/15 screen above framed "own the bottleneck" as the smart, hedged, less-crowded expression of the
power thesis. A bull/bear stress-test (see [[how-to-get-paid]], debate 2026-07-15) broke that framing.
**Corrected formulation (Jake, verbatim):**
> Bottleneck equities are not hedges against an AI-capex slowdown merely because they benefit from
> scarcity. When their revenues depend on the same hyperscaler spending cycle—and their valuations
> already capitalize persistent shortages—they are often higher-beta expressions of that cycle. In a
> deceleration, pure-play suppliers can derate faster than diversified builders with durable underlying
> cash flows. **Treat bottleneck ownership as a selective return thesis, not portfolio insurance.**
- Own-receipts: the 7/15 screen itself showed VRT +143% / POWL +241% / GEV +88% at 40–80x EV/EBITDA near
  highs — the highest-BETA leg of the capex cycle, not a hedge against its reversal. The bottleneck
  owner's revenue IS the builder's expense: opposite ends of one transaction, not two risk factors.
- Implication for the screen: `gap_score` still finds *relatively cheap* within the layer, but "cheap
  bottleneck" is a cheaper bet on the SAME cycle, not insurance against it. The real hedges live
  elsewhere ([[how-to-get-paid]]: cash, gold, diversified quality).

### 2026-07-17 ~4:39pm PT — PJM auction: grid scarcity CONFIRMED official (7GW short, 500MW new supply)
Source: UtilityDive/ZeroHedge (Jake paste). PJM = largest US grid operator, 67M people, Mid-Atlantic+Midwest
(= "data-center alley" / Virginia). Jake: "one of those 'well duh' moments in 2 years."
#### DATA (observed)
- PJM capacity auction cleared **~7 GW BELOW its reliability target**; drew only **~500 MW of NEW supply.**
- **FERC Chair Swett: "alarm bells... PJM failed to deliver"**; stakeholder process "slow where it must be
  fast." Commissioner LaCerte (last week): status quo "untenable." Tech conference Jul 23 on PJM governance.
- **FERC set NERC deadlines for data-center/crypto ("computational load") RELIABILITY STANDARDS + registration
  rules by Dec 31**; proposed standards out for comment in August (NERC Large Loads Action Plan).
#### THESIS (interpretation — NOT fact)
- *(analysis — thesis confirmed)* Official, hard confirmation of the [[power-not-petroleum]] structural
  scarcity thesis: the grid is short of RELIABILITY (not just growth), and **new supply is NOT coming** (500MW
  ≈ nothing vs a 7GW hole). The binding constraint on the AI buildout is physical and getting worse, not better.
- *(analysis — cuts the compression heal)* This CHALLENGES the "power eases (turbines now, nuclear/SMR behind)"
  leg of the [[compression-thesis]] input-deflation heal. If capacity is NOT catching up (PJM says it isn't),
  power stays a scarce, expensive input → the hyperscaler-PAYERS' margin squeeze on the power line does NOT
  resolve, and the input-SELLERS' scarcity rent persists longer. A point AGAINST the benign "supply catches
  up" branch, at least on the power leg.
- *(analysis — the AI-capex constraint gets a regulatory clock)* The NERC data-center reliability standards
  (Dec 31) = grid regulators formally treating data-center load as a destabilizer → interconnection limits/
  curtailment risk = a potential brake ON the buildout itself (can't deploy capex you can't power). Watch the
  August proposed standards.
- *(⚠️ discipline — scarcity REAL ≠ scarcity-equities CHEAP)* The bottleneck≠hedge correction above still
  holds: PJM confirms the scarcity is real, but the power/IPP/equipment names already capitalize it (VRT/POWL/
  GEV 40–80x, and fell hardest 7/16). **Real structural tailwind, likely-priced trade.** And WARNING-vs-TRIGGER:
  grid inadequacy is a multi-YEAR STATE (permitting/build takes a decade) — Jake's "well duh in 2 years" is
  exactly right: obvious in hindsight, slow-burn, times NOTHING. A structural confirmation, not a dated fuse.

## 2026-08-19 ~10:20am PDT — ⭐⭐⭐⭐⭐ **THE NAMED-COUNTERPARTY MAP THE VAULT DID NOT HAVE: 22 hyperscaler→power contracts with capacity, term and status — plus an EVIDENCE LADDER for capex reality that immediately downgrades two names this note carries**
Source: Jake's compiled research, 2026-08-19 → `raw/2026-08-19-dc-power-money/brief.md`.
⚠️ **ARTIFACT TEST: compiled SECONDARY research with per-claim attributions. Independently verified: NONE.
Everything below is REPORTED.** Structural consequences filed at [[transmission-chain]]`:L71`.
**⚠️ EVERGY (EVRG) HAS NO PRIOR MENTION IN THIS VAULT — a signed ~2.5 GW hyperscaler book was invisible.**

### DATA (REPORTED) — the direct power counterparties, by tier
- **NUCLEAR / IPP (this note's tier 1):** **Amazon→TALEN up to 1,920 MW through 2042** (Susquehanna; full
  volume no later than 2032) · **Microsoft→CONSTELLATION ~835 MW 20yr** (Crane/TMI-1 restart target 2028) ·
  **Meta→VISTRA >2.1 GW 20yr** (Perry + Davis-Besse; uprates ≈ +433 MW early 2030s) · **Meta→CONSTELLATION
  1,121 MW 20yr from 2027** (Clinton, +~30 MW uprate) · **Google→NEXTERA 615 MW 25yr** (Duane Arnold restart
  target Q1-2029).
- **REGULATED UTILITY (tier 2):** **Meta→ENTERGY LOUISIANA** up to 5 GW (Richland Parish/Hyperion) ·
  **Oracle/Stargate→DTE ~1.4 GW**, approved Dec-2025, ≥17yr · **Google→DTE 1.0 GW through Dec-2047** ·
  **Google+Meta+Beale→EVERGY ≈2.5 GW aggregate signed peak** · **Amazon→AEP OHIO** · **Equinix→CENTRAL
  GEORGIA EMC 20yr** (Aug-2026) · **xAI→MLGW/TVA ~150 MW initial, operating.**
- **BEHIND-THE-METER / CO-LOCATED:** **Microsoft→CHEVRON/Energy Forge One ~2.67 GW 20yr** (Project Kilby,
  W-Texas, signed 6/22/26, first power 2028) · **Oracle→BLOOM 1.2 GW contracted, framework to 2.8 GW.**
- **POWERED LANDLORD — a tier this note did not have:** **CoreWeave→GALAXY DIGITAL 526 MW critical IT
  (~800 MW gross) at Helios, 15yr leases → GALAXY→AEP TEXAS/WETT, >1.6 GW ERCOT-approved.**
- **SECOND CHECK (equipment):** GEV majority of Kilby turbines; **~116 GW gas backlog PLUS SLOT RESERVATIONS**,
  **>$5B DC Electrification orders H1-2026, >2× FY2025** · CAT/Solar Turbines on Kilby, plus **~2 GW of gas
  gensets for Monarch Compute Campus, deliveries Sep-2026→Aug-2027** · **LG Energy Solution Vertech ~$1.6B,
  1.5 GW / 6 GWh with DTE**, a major portion tied to the Oracle load · Bloom also **AEP up to 1 GW** (initial
  100 MW) and **AWS/AEP Ohio 72.9 MW, service 2027** · **Generac $1.6B DC backlog** but **⬜ no named
  customers or campuses.**
- ⚠️ **BROOKFIELD/BLOOM "$25B" IS A FINANCING FRAMEWORK, NOT BLOOM CONTRACTED REVENUE.** Same for
  **Microsoft/Brookfield >10.5 GW** — a portfolio procurement program, **not** 10.5 GW on one campus.

### ⭐ THE EVIDENCE LADDER FOR CAPEX REALITY — adopt this; it is WARNING-vs-TRIGGER for buildouts
**ESA → large-load tariff (reveals minimums/duration/termination) → PPA → interconnection/service agreement
→ state PUC approval → customer-funded substation/transmission filing → GAS-TURBINE SLOT RESERVATION →
limited/full NOTICE TO PROCEED → FINAL INVESTMENT DECISION → EQUIPMENT PURCHASE ORDER → take-or-pay.**
**The further down, the less speculative.**

### THESIS (interpretation — NOT fact)
- **⛔⛔ THE LADDER IMMEDIATELY DOWNGRADES TWO THINGS, AND ONE OF THEM IS THE DOCUMENT'S OWN HEADLINE
  EXAMPLE. MICROSOFT→CHEVRON 2.67 GW IS **PRE-FID** — the PPA is signed but Chevron expected final
  investment decision only "later in 2026."** ⇒ **A signed 20-year PPA with no committed project capital is
  MID-ladder, not bottom.** ⇒ **Do NOT carry 2.67 GW as committed capacity, and do not carry the GEV turbine
  award as a booked order.** **Second: the B&W/Siemens ~1 GW Applied Digital project was announced under a
  LIMITED NOTICE TO PROCEED — below Oracle/Bloom and Microsoft/Chevron in certainty by the ladder's own
  ordering.** *(Analysis.)*
- **★★★★★ AND IT GRADES GEV PRECISELY, WHICH THIS NOTE'S TIER-3 FRAMING COULD NOT: "~116 GW of backlog PLUS
  SLOT RESERVATIONS" IS TWO DIFFERENT OBJECTS REPORTED AS ONE NUMBER.** **A slot reservation sits ABOVE a
  purchase order on the ladder — it reserves manufacturing capacity, it is not revenue.** ⇒ **The
  >$5B of DC Electrification orders at >2× FY2025 is the harder half and it is the one to track.**
  ⇒ **This is consistent with [[ai-capex-cycle]]`:L2717`, which put GEV on the QUANTA/DYCOM (cancellable
  order-book) side of the 2000 split, not the Crown Castle side.** *(Analysis.)*
- **★★★★★★ THE TIERING IN THIS NOTE NEEDS ONE CORRECTION AND IT REVERSES A RANKING. `:L38` CALLS REGULATED
  UTILITIES "LOW BETA… THE DEFENSIVE EXPRESSION" BECAUSE THEY EARN ON RATE BASE AND DO NOT CAPTURE PRICE
  SPIKES. THAT IS STILL TRUE ON PRICE — AND IT MISSES WHAT CHANGED: THEY NOW CAPTURE **VOLUME** UNDER
  20-45 YEAR CONTRACTS WITH MINIMUM-PAYMENT FLOORS.** **Evergy: minimums covering ≥80% of contracted
  capacity, plus collateral and termination charges.** ⇒ **"Low beta to scarcity PRICING" and "high,
  contractually protected exposure to AI VOLUME" are different statements, and this note only made the
  first.** ⇒ **On a bust, the merchant IPP loses the capacity price; the regulated utility with an 80%
  minimum does not lose the volume.** *(Analysis. Correcting this note's own tier logic.)*
- **⚠️ AND THE HONEST DISCOUNT ON ALL OF IT: THIS IS ONE COMPILED DOCUMENT, UNVERIFIED, AND ITS OWN CLOSING
  CAVEAT IS THE RIGHT ONE — THE PUBLICLY VISIBLE CONTRACTS ARE A FLOOR, NOT A MAP.** **Gas commodity
  suppliers, firm pipeline transportation, transformer/switchgear OEMs, EPC contractors and many utility
  customer names are confidential.** ⇒ **Any "who gets paid" ranking built from public contracts is biased
  toward the disclosed — which is exactly why Generac looks weaker than Bloom here: not less real revenue,
  less visible customers.** *(Analysis.)*

### 📌 REGISTERED
1. 🚩🚩🚩 **EVERGY (EVRG) — OPEN COVERAGE.** ~2.5 GW signed, ≥80% minimums. **Verify from the 10-Q and the
   large-load tariff.** **A name with a contracted hyperscaler book was invisible to this vault until today.**
2. 🚩🚩 **GRADE EVERY POWER DEAL IN THIS NOTE AGAINST THE LADDER** — signed-vs-PUC-approved-vs-NTP-vs-FID.
   **The tiering above is by TECHNOLOGY; the ladder sorts by CERTAINTY, and they disagree.**
3. 🚩🚩 **GALAXY DIGITAL (GLXY) as a POWERED LANDLORD** — a tier this note lacks entirely. **Contracted power
   monetised through the lease, without being a utility.** ⬜ No prior vault coverage.
4. 🚩 **DOES THE MICROSOFT/CHEVRON FID LAND IN 2026?** **A dated, falsifiable trigger on the largest
   co-located project in the document, and it decides whether the GEV turbine award is real.**
**Links:** [[transmission-chain]] · [[power-not-petroleum]] · [[ai-capex-cycle]] · [[buildout-bottleneck-map]] · [[nuclear]]

---

## 2026-08-22 ~9:00am PDT — ⭐⭐⭐⭐ **NVDA BOUGHT INTO THE POWER BOTTLENECK ITSELF (CLOVERLEAF), AND PENNSYLVANIA TURNED DATA-CENTRE BACKLASH INTO A PERMITTING REGIME THE SAME DAY. STAGE 4b AND STAGE 4c BOTH MOVED, IN OPPOSITE DIRECTIONS, ON THE SAME FRIDAY.**
Source: Jake's DAILY HOLDINGS SCAN 2026-08-22 → `raw/2026-08-22-holdings-scan/`.
[S1/S2] Cloverleaf newsroom 8/21 · [S8/S9] Commonwealth of Pennsylvania 8/21.

#### DATA (REPORTED — company disclosure)
- **NVIDIA took a MINORITY INVESTMENT in Cloverleaf Infrastructure + a strategic partnership.**
  **⬜ TERMS NOT DISCLOSED — there is no size, no stake, no board seat in this. Treat the SIZE as
  unknown, not small.**
- **Cloverleaf's business is exactly the vault's named bottleneck: land · grid access · power
  availability · permitting · cooling · shovel-ready sites.** **Claims multiple GW-scale projects
  across North America** *(claimed, not verified — ⬜ no project list, no interconnection queue
  positions).*
- **NVIDIA exec Nico Caprez: "land, power and shell" are the foundation of AI factories.**

#### ★★★★ THE FINDING: THE SUPPLIER-AS-BANK PATTERN JUST EXTENDED INTO SUPPLIER-AS-DEVELOPER
- **[[ai-financing-fragility]]`:L6898` established three-for-three on 8/20: NVDA guarantees $500B ·
  MRVL grants Google warrants · AVGO would guarantee $60-70B. ⇒ "THE CHIPMAKERS HAVE BECOME BANKS."**
- ⇒ **★★★ CLOVERLEAF IS THE NEXT RUNG: NVDA IS NOW FUNDING THE *PHYSICAL PRECONDITION* OF ITS OWN
  DEMAND, not just the customer's balance sheet.** ⇒ **The seller is no longer financing the
  purchase — it is financing the SITE THE PURCHASE WILL SIT ON.**
- ⇒ **⚠️ AND IT IS A DEMAND-SIDE TELL, WHICH IS THE UNCOMFORTABLE READING: a company does not buy
  into land and permitting because chip demand is the constraint. It does so because it has
  concluded the constraint is PHYSICAL and will bind for long enough to be worth owning.**
  ⇒ **★ THAT IS BULLISH FOR POWER SCARCITY AND AMBIGUOUS FOR NVDA: it confirms the bottleneck is
  real, and it is what a seller does when it fears its order book is capped by something it does
  not control.** *(Analysis. Reads against the scan's straightforwardly positive framing.)*

#### ⭐⭐ PENNSYLVANIA — THE VAULT'S "BIPARTISAN RATCHET" NOW HAS A MECHANISM
- **Executive Order 2026-05 · GRID framework · public data-centre portal launched 8/21.**
  **Requirements: local zoning approval FIRST, then the expedited state pathway · energy
  affordability · community involvement · environmental protection · transparency · workforce
  commitments.**
- **⭐⭐⭐ THE LOAD-BEARING PROVISION: developers are pushed to BRING NEW POWER rather than consume
  existing grid capacity and pass infrastructure costs to existing ratepayers.**
- ⇒ **★★★★ THIS IS THE COST-ALLOCATION FIGHT RESOLVING AGAINST THE DEVELOPER, IN WRITING, IN A PJM
  STATE.** ⇒ **The vault's "bipartisan, therefore a ratchet" call gets its first concrete
  instrument.** ⇒ **AND IT IS NOT A BAN — it is a COST. That distinction is the whole read: it
  raises the price and the timeline of new supply, which makes EXISTING dispatchable generation
  in PJM more valuable, not less.** **CEG · TLN · VST are the direct beneficiaries; GEV benefits
  from the "bring your own power" mandate creating equipment demand.** *(Analysis.)*
- **⇒ ⛔ AND THE DISTINCTION THE VAULT SHOULD NOW CARRY EVERYWHERE, which is the scan's best line:
  ANNOUNCED GW ≠ PERMITTED GW ≠ ENERGIZED GW.** ⇒ **Every capacity figure in this vault should be
  tagged with which of the three it is. Most press numbers are ANNOUNCED.** 🚩 **Registered: audit
  the vault's held GW figures against that three-way split.**

#### ⚠️ THE HONEST TENSION BETWEEN THE TWO ITEMS
- **NVDA is buying INTO site development on the same day a major PJM state made site development
  slower and more expensive.** ⇒ **Either NVDA is buying the scarcity (rational — the permitting
  moat makes shovel-ready sites MORE valuable), or it is buying into a headwind it has not priced.**
  ⇒ **⬜ Cloverleaf's project list and interconnection positions would separate those, and neither
  is disclosed.**

### 2026-09-08 ~10:50pm PDT — **THE SOCAL HEATWAVE IS A DISCONFIRMER OF THE GRID-FRAGILITY FRAME, NOT AN EXAMPLE OF IT: 10-15°F ABOVE NORMAL PRODUCES A CAISO PEAK AT 91% OF A FOUR-YEAR-OLD RECORD, AND THE "SURGING" PRICE IS THE HIGHEST IN *ONE WEEK*.** *(ZH/Bloomberg/Arcus-NRGStream, RSS full text)*
#### DATA
- **CAISO forecast peak: 47,379 MW Wednesday · 45,183 MW Thursday. September 2022 record: 52,061 MW.** CAISO serves ~80% of California + a slice of Nevada.
- LA-area temps **85-105°F, ~10-15°F above normal**; SF 86°F Wed; Sacramento 100°F Thu; statewide max peaks **Thursday ~95°F**, then **~75°F by mid-month**.
- **SP15 (SoCal hub) day-ahead for Tuesday's 6pm hour: $87.69/MWh — "the highest hourly reading in a little over a week."**
- ⚠️ NWS heat advisories Central Valley + coastal; extreme-heat warnings parts of SoCal. Stress framed as **evening, as solar generation declines**.

#### THESIS (interpretation — NOT fact)
- **★★★★ DECOMPOSE BEFORE READING — THE STANDING CHECK, AND IT REVERSES THE HEADLINE. 47,379 / 52,061 = 91.0% of the record, on a day 10-15°F above normal.** ⇒ **CAISO has ~4.7 GW (9%) of headroom against its own high-water mark during an anomalous heat event.** **That is not a system at the edge; it is a system with margin.** **The vault's power-scarcity thread is about MULTI-YEAR load growth from data centres, and this print is evidence that California's peak has NOT grown into its 2022 record in four years.** *(Analysis. ⚠️ Peak ≠ reserve margin — the binding constraint is dispatchable capacity at the evening ramp, not the peak MW number. ⬜ CAISO's actual reserve margin and any Flex Alert / EEA notice, which is the datum that would change this read.)*
- **⛔ "$87.69, THE HIGHEST IN A LITTLE OVER A WEEK" IS A SUPERLATIVE ON A SEVEN-DAY WINDOW AND SHOULD BE READ AS THE ABSENCE OF A PRICE STORY.** SP15 scarcity pricing runs in the hundreds to thousands per MWh; **$87.69 is an ordinary summer evening.** ⇒ **when a wire reaches for "highest in a week," the honest translation is "nothing here."** **✓ Same class as the group-mean check and the read-the-caption-first check: the superlative's WINDOW is the tell.**
- **★★★ WHAT IS GENUINELY STRUCTURAL AND IS BURIED IN ONE CLAUSE: *"particularly in the evening as solar generation declines."*** **The duck curve is the real constraint and it is present on normal days too — the heatwave only makes it visible.** ⇒ **the investable read is unchanged and is about the EVENING RAMP: dispatchable gas peakers, storage duration, and transmission — not about summer peak MW.** **This is why the China battery-storage approval halt filed tonight matters more to this thread than the heatwave does.** → [[buildout-bottleneck-map]].
- **⚖️ THE OTHER SIDE, HELD HONESTLY: a forecast is not an outcome.** If Wednesday's actual exceeds the forecast, or CAISO issues a Flex Alert, the read flips from "margin" to "forecast error under stress" — and forecast error under stress is itself the fragility. **⬜ Wednesday's realized peak vs 47,379 is a cheap, dated check.**
#### GRADES (16c) → [[grades]]
**No moves. A four-day weather event does not clear the 120-day bar on any power name.** Direction: **mildly NEGATIVE for the "California grid is about to break" version of the power-scarcity story; NEUTRAL for the evening-ramp/duration version, which this does not test.**
**📌 REGISTERED:** ⬜ **Wednesday's REALIZED CAISO peak vs the 47,379 forecast** · ⬜ **any Flex Alert / EEA-1 notice** · ⬜ CAISO reserve margin (the number that actually governs) · SP15 evening-hour prints Wed/Thu vs $87.69.
**Links:** [[buildout-bottleneck-map]] · [[power-not-petroleum]] · [[nuclear]]

## 2026-09-10 ~8:45am PDT — ⭐⭐⭐⭐⭐⭐ **JAKE'S STANDING ENERGY THESIS, TESTED AGAINST SEMIANALYSIS'S SUPPLY-CHAIN DATA: *"it's just too scattered and unpredictable for any growth to really concentrate."* **THE DATA SUPPORTS HIM AT THE EQUIPMENT LAYER AND SHOWS WHERE THE CONCENTRATION ACTUALLY WENT — AND THE ANSWER MAKES HIS THESIS *STRONGER*, NOT WEAKER, BECAUSE THE CONCENTRATING LAYER IS MOSTLY PRIVATE.**
### ✔ THE EVIDENCE FOR HIS THESIS, FROM THE SOURCE'S OWN NUMBERS
1. **THE SUPPLIER BASE IS FRAGMENTING, MEASURED: "a year ago we counted **12 distinct manufacturers** that had secured multi-hundred-MW datacenter off-grid orders; **today that count is 22 and will continue to grow.**" They track **>30 OEMs.**** ⇒ **the winner count is nearly DOUBLING annually. That is the opposite of consolidation.**
2. **AND THE INCUMBENTS DID NOT CAPTURE IT — STATED OUTRIGHT: "winners have **NOT ONLY** been the incumbents like **GEV and Siemens Energy.** The **BIGGEST beneficiaries** have been the **dozens of suppliers of reciprocating engines.**"** ⇒ **⚠️ that is a mild NEGATIVE to the vault's GEV 7 and it is the source's own framing, not mine.**
3. **THE PRODUCT ITSELF IS UNSTANDARDISED: solutions from **0.5MW per unit to 20MW per unit** adopted at scale — a **40× range** — spanning recips, aeroderivative turbines, J-class turbines and fuel cells.**
4. **SIX INDEPENDENT GATES, ANY ONE OF WHICH KILLS A PROJECT: contract & bankability · permits · fuel · equipment · workforce · electrical physics.** SemiAnalysis: *"our permit-level tracking shows booked capacity facing difficulties at NEARLY EVERY ONE OF THEM."*
5. **AND THE REGULATORY SURFACE IS COUNTY-LEVEL, NOT NATIONAL: the same equipment faces a **10× different per-source MW ceiling** depending on the county (~259 MW in an attainment area vs **~26 MW in Dallas or Houston**).** ⇒ **⇒ THAT IS "SCATTERED" AS ARITHMETIC.**
6. **CONVERSION IS 4%: 75GW of firm binding orders against ~3GW operational by year-end.**
⇒ **📌 ON THE EQUIPMENT LAYER JAKE IS RIGHT AND THE DATA IS UNAMBIGUOUS. A fragmenting supplier base, an unstandardised product, six serial kill-gates and a county-by-county regulatory surface is a structure in which no single manufacturer compounds.**
### ⇒ ⭐⭐ BUT SEMIANALYSIS ALSO IDENTIFIES WHERE CONCENTRATION *IS* HAPPENING, AND IT IS ONE LAYER UP
- **"All of that complexity has led to the development of a NEW TYPE OF SERVICE PROVIDER: **Energy-as-a-Service (EaaS) vendors. THESE ARE THE NEW BTM UTILITIES.** Companies such as **VoltaGrid deliver POWER, not just equipment**, under long-term contracts with **guaranteed capacity and up-time commitments.** EaaS can do everything from leasing gensets to **trucking compressed natural gas to site as VIRTUAL PIPELINES** while permanent plants are permitted and built."**
- **"**VoltaGrid** has proved to be one of the winners of that trend, landing **MULTIPLE GIGAWATTS of orders** with top-tier developers like **Vantage Data Centers.** Others like **WILLIAMS** and **SOLARIS** are also on the rise and have **scored massive contracts.** Separately, some manufacturers are increasingly doing that themselves, such as **BLOOM ENERGY** and **ENCHANTED ROCK.**"**
- **★★★★★★ THE SYNTHESIS, AND IT IS JAKE'S OWN AXIS APPLIED TO ENERGY: THE SCARCE THING IS NOT THE TURBINE. IT IS THE ABILITY TO **DELIVER A MEGAWATT UNDER CONTRACT WITH AN UPTIME GUARANTEE.*** ⇒ **the equipment is commoditising (12 → 22 → more suppliers) while the INTEGRATION-AND-GUARANTEE layer concentrates into a handful of "new BTM utilities."** ⇒ **⇒ VALUE ACCRUES TO WHOEVER SELLS THE *OUTCOME*, NOT THE *COMPONENT* — which is exactly the seller-denomination correction Jake made to this vault on the compute stack, arriving independently in energy.** **✔ SAME SHAPE AS COMPUTE: 22 equipment suppliers proliferating while NBIS auctions DELIVERED compute 15-20% above list. Two stacks, one structure.**
### ⛔ AND HERE IS WHY THAT MAKES HIS THESIS STRONGER RATHER THAN WEAKER
- **THE CONCENTRATING LAYER IS LARGELY *PRIVATE*. VoltaGrid: private. Enchanted Rock: private. Vantage: private (DigitalBridge). Crusoe: private.** ⇒ **⇒ THE ENERGY BUILDOUT FRAGMENTS AT THE LAYER YOU CAN BUY AND CONCENTRATES AT THE LAYER YOU CANNOT.** **⇒ BOTH ROUTES LEAD TO JAKE'S CONCLUSION FOR A LISTED PORTFOLIO, AND THE SECOND IS THE STRONGER ARGUMENT. He should have it.**
- **THE LISTED EXCEPTIONS ARE THREE, AND TWO ARE ABSENT FROM THIS VAULT:**
  | name | status here |
  |---|---|
  | **BE (Bloom Energy)** | **GRADED 6** (9/2, updated 9/4) — ⛔ and I claimed twice it was "not on the roster." It is. |
  | **WMB (Williams Companies)** | **ABSENT.** ⚠️ "Williams" in this vault is **JOHN WILLIAMS of the NY Fed** — 7 files, zero of them the pipeline company. |
  | **SEI (Solaris Energy Infrastructure)** | **⬜ AMBIGUOUS AND WORTH CHECKING.** The vault's only "Solaris" is the **SpaceX "Solaris/Stateline VIE"** (`balance-sheet-board:L310`, Solaris 50.1% / MZX Tech 49.9%). **If that is the same Solaris, the vault already holds a power-services name inside a SpaceX structure and never labelled it as one.** 🚩 |
  | VoltaGrid · Enchanted Rock · Vantage · Crusoe | **PRIVATE — 0-2 files each; VoltaGrid ZERO.** |
- **⚖️ THE HONEST COUNTER TO JAKE, STATED SO HE HAS BOTH SIDES: the EaaS layer is EARLY, and early-stage concentration often disperses once the service is standardised — trucking CNG as a "virtual pipeline" is a bridge business, not obviously a durable moat. And a fragmenting supplier base is exactly what a genuine boom looks like BEFORE consolidation; the question is whether this fragments permanently or consolidates in three years.** ⇒ **the falsifier for his thesis is the OEM count: it went 12 → 22 in a year. If it goes 22 → 15 next year, concentration is arriving. If it goes 22 → 35, he is right and it is structural.** 🚩 **REGISTERED AS THE TEST — and SemiAnalysis publishes the count, so it is checkable annually.**
### GRADES (16c) → [[grades]]
**No moves.** **⚠️ GEV 7 — the source says explicitly the incumbents were not the biggest beneficiaries; that is a recorded caution inside the existing grade, not a cut. BE 6 hold — named twice as a pivot destination AND as an EaaS provider in its own right, which is a better position than "equipment supplier," but the 9/2 cap (execution/dilution history) is untouched.**
**📌 REGISTERED:** 🚩 **the OEM count next year — 22 → fewer = consolidation; 22 → more = Jake is right structurally. THE test for his thesis and it is published annually** · 🚩 **⬜ is the SpaceX "Solaris/Stateline VIE" the same Solaris Energy Infrastructure? If so the vault holds it and mislabelled it** · ⬜ **WMB's datacenter power contracts — genuinely absent from the book** · ⬜ VoltaGrid's ownership and whether any listed vehicle carries it.
**Links:** [[buildout-bottleneck-map]] · [[compression-thesis]] · [[grades]] · [[transmission-chain]]
- **Addendum 2026-09-10 ~9:00am PDT — **IREN'S ANNOUNCED CAMPUS PLANS NOW SPAN TWO CONTINENTS AND ~2,800 MW IN THREE DAYS OF SCANS, WHILE ITS DELIVERABLE BY Q4-2027 IS ~300 MW — 11%.** NVIDIA NAMES IREN IN AN AUSTRALIAN AI-FACTORY BUILDOUT "UP TO 2GW BY 2027," APPLYING THE DSX BLUEPRINT TO IREN'S **800MW BUNDEY CAMPUS** (South Australia).** *(NVIDIA release 8:00pm ET 9/9; partners Firmus, Sharon AI, IREN, Megaport, ResetData, CDC, NEXTDC, AirTrunk. Sharon AI separately plans up to 68,000 GPUs.)*
- **✔ THE SCAN'S OWN DISCIPLINE IS EXACTLY RIGHT AND IS ADOPTED VERBATIM: "the 2GW is the AGGREGATE Australian ecosystem target; NVIDIA did NOT assign 2GW to IREN and disclosed **no contract value, no GPU purchase volume and no minimum revenue commitment.**"** ⇒ **that is the vault's own "MW DO NOT ADD" rule applied by the source before the board had to apply it. Rung: NAMED PARTNER, not contracted revenue.**
- **⭐ THE LADDER, ACROSS THREE DAYS, AGAINST THE SCAN'S OWN VALUE HIERARCHY — "energized MW > firm interconnection > permitted generation > announced GW":**
  | filed | site | MW | rung |
  |---|---|---|---|
  | 9/9 | Sweetwater 1+2, ERCOT Batch Zero | **2,000** | **CONDITIONAL classification** — not final interconnection |
  | 9/9 | ↳ gross DC capacity targeted Q4-2027 | **300** | targeted |
  | 9/9 | ↳ Sweetwater 1 substation | — | **ENERGIZED** — the only top-tier item |
  | 9/10 | Bundey, South Australia | **800** | **ANNOUNCED** — bottom tier |
  ⇒ **~2,800 MW announced; ~300 MW deliverable by Q4-2027 = **11%.*** ⇒ **⛔ BUNDEY SITS AT THE BOTTOM OF THE HIERARCHY THE SCAN ITSELF PUBLISHES, AND THE ONE ITEM AT THE TOP — AN ENERGIZED SUBSTATION — WAS BURIED THIRD IN YESTERDAY'S RELEASE.**
- **★★★★ AND THE TENSION IS THE POINT, NOT THE HEADLINE: **IREN'S ANNOUNCED GIGAWATTS KEEP GROWING AND ITS GRADE WAS CUT (6→5) ON FINANCING.** Nothing in three days of announcements addresses financing. Meanwhile the same scan puts the 10Y at ~4.90%, PPI at +5.4% and Brent above $101, and names IREN explicitly in its own negative financing read-through.** ⇒ **⇒ THE ANNOUNCEMENTS AND THE CONSTRAINT ARE MOVING IN OPPOSITE DIRECTIONS ON THE SAME NAME, ON THE SAME DAY, IN THE SAME DOCUMENT.** **⇒ IREN 5 HOLDS. What would move it is the Sweetwater and Bundey FUNDING structures, not another named partner.** 🚩 **⬜ how Bundey is capitalised — and note Australia adds currency and jurisdiction risk to a balance sheet already financing at ~9% vs Lambda's Baa2 at ~6.64%.**
- **⭐ NVDA read: this is PLATFORM LOCK-IN expressed as geography — DSX as a reference architecture propagated through named regional partners, at no disclosed cost to NVIDIA.** ⇒ **the same structure as the warrants-for-volume family in a non-financial form: NVIDIA supplies the blueprint and the brand, the partner supplies the balance sheet and the megawatts.** **NVDA 6 hold.**

## 2026-09-12 ~9:40am PDT — ⭐⭐⭐⭐⭐⭐ **THE D.C. CIRCUIT JUST NARROWED §202(c) — THE EXACT AUTHORITY THIS NOTE HAS ON FILE AT `:L34` — AND IT WEAKENS THE **COAL-REVERSAL DISCONFIRMER** TO VST/CEG/TLN ON ITS **SECOND** LEG. ⇒ **AND UNLIKE THE ENERGY-SECURITY READ I REJECTED THIS MORNING, THIS ONE IS A **CASH-FLOW CHANNEL**, NOT A NARRATIVE ONE.*** *(AP via Jake's Daily Market Brief 9/12. Unanimous panel; J.H. Campbell, Michigan.)*

### DATA
- **D.C. Circuit, unanimous: DOE EXCEEDED ITS AUTHORITY using Federal Power Act §202(c) to force Michigan's J.H. Campbell coal plant to stay open past its planned retirement.** The panel: **§202(c) is a NARROW, LAST-RESORT authority requiring a GENUINE IMMEDIATE EMERGENCY — not a broad reliability concern.**
- **Scope beyond one plant: the administration declared a national energy emergency partly BECAUSE of AI/data-centre load, and DOE has used the same power to keep retiring plants running in INDIANA, COLORADO and FLORIDA.**
- **⚠️ AND HOURS AFTER LOSING, Energy Secretary Chris Wright issued ANOTHER §202(c) order — CENTRALIA, WASHINGTON.** **Campbell itself is STILL OPERATING under a recent order while the utility reviews the ruling. This is not a shutdown order.**

### ⇒ WHAT IT DOES TO THE VAULT'S STANDING POSITION
- **`grades.md:154` carries the disconfirmer to the merchant-nuclear thesis as: *"a US coal-policy reversal unlocks 248 Bt"* — and Jake's 9/10 correction already halved it: *"that is PARTLY PHYSICAL (plants demolished, not mothballed), so slower than a decree"* (`oil-value-chain:2972`).**
- ⇒ **★★★★ SO THE DISCONFIRMER HAD TWO LEGS AND **BOTH ARE NOW IMPAIRED: PHYSICAL** (retired plants are demolished — you cannot decree a demolished boiler back) **AND LEGAL** (the emergency authority for keeping the *not-yet-retired* ones alive has been read narrowly by a unanimous panel).** ⇒ **📌 THE SCARCITY OF EXISTING FIRM GENERATION IS MORE DURABLE THIS MORNING THAN IT WAS YESTERDAY, AND FOR A REASON THAT RUNS THROUGH CASH FLOWS RATHER THAN SENTIMENT.**
- ⇒ **⭐ AND THE CONTRAST WITH WHAT I REJECTED 40 MINUTES AGO IS THE POINT: I struck down the holdings scan's *"Middle East disruption ⇒ positive VST/CEG/TLN"* because **US gas is landlocked and merchant power prices off gas, not Brent** — a narrative channel. **THIS is the version that works: it changes the SUPPLY of dispatchable MW that competes with them, which is a price-setting variable.** 📌 *Same conclusion, completely different mechanism — and only one of them belongs in a driver line.*

### ⛔ CLASSIFY THE CONSTRAINT BY WHAT MUST CHANGE FOR IT TO LIFT — AND DO NOT OVERSTATE IT
**A §202(c) narrowing lifts via: (i) Supreme Court reversal · (ii) Congress amending the FPA · (iii) a genuinely declared immediate emergency · (iv) a DIFFERENT statutory authority.** ⇒ **that is a slow list, which is what makes it durable. ⚠️ But it is ONE circuit, it constrains §202(c) SPECIFICALLY, and nothing is shutting today.**
- ⇒ **★★★ THE WRIGHT/CENTRALIA ORDER **HOURS AFTER** THE RULING IS THE MOST INFORMATIVE DETAIL AND IT IS NOT A LEGAL STRATEGY — IT IS A STATEMENT OF INTENT TO KEEP ISSUING ORDERS AND LITIGATE EACH ONE.** ⇒ **📌 that converts the policy tool from **RELIABLE** to **CONTESTED**: each plant now needs its own order and its own lawsuit, on a clock set by courts rather than by DOE.** ⇒ **⇒ THE HONEST READ IS THE SAME SHAPE AS THIS MORNING'S PIPELINE CALL: **MATERIAL, NOT SYSTEMIC.** The tool degrades; it does not vanish.**
- ⛔ **AND THE SECOND-ORDER RISK CUTS THE OTHER WAY FOR THE VAULT'S POWER SLEEVE: if forced retention fails, the administration's alternative is to ACCELERATE PERMITTING AND INTERCONNECTION for new build.** ⇒ **that is bullish GEV/ETN/PWR (the brief says so) but it is the **long-run DISCONFIRMER to scarcity pricing** — new MW is exactly what compresses the premium VST/CEG/TLN earn.** 📌 **16c(c): that arrives well beyond 120 days, so it moves no grade today — but it is the mechanism to watch, and it is the OTHER side of the same ruling.**

### ⛔ A ROSTER HOLE THIS EXPOSES
- **The brief names **NRG** alongside CEG/VST. `:L10` and `:L18` of this very note list NRG as a **TIER-1 MERCHANT IPP** — and `grep` confirms **NRG HAS NO ROW IN `grades.md`.*** ⇒ **📌 a tier-1 name in the vault's own power taxonomy, ungraded, surfacing in a third-party brief on a thesis the vault owns.** 🚩 **⬜ NRG — add to the tranche backlog; it is a cheaper hole to close than the cybersecurity one because the thesis is already written here.** *(Also ungraded from this note's tier 1: PSEG.)*

**📌 REGISTERED:** ⭐⭐⭐⭐ **D.C. CIRCUIT NARROWS §202(c) (unanimous): a narrow last-resort authority needing a GENUINE IMMEDIATE EMERGENCY, not broad reliability — the exact tool on file at `:L34`** · ★★★★ **THE COAL-REVERSAL DISCONFIRMER IS NOW IMPAIRED ON BOTH LEGS — PHYSICAL (demolished, per Jake 9/10) and LEGAL (this ruling) ⇒ existing firm-generation scarcity is more durable than it was yesterday, through a CASH-FLOW channel** · ⭐ **CONTRAST WITH MY OWN 9:00am REJECTION: "Middle East oil ⇒ VST/CEG/TLN" is narrative (US gas is landlocked); THIS changes the SUPPLY of competing dispatchable MW, which is price-setting — same conclusion, only one mechanism belongs in a driver line** · ★★★ **WRIGHT ISSUED A NEW §202(c) ORDER (CENTRALIA) HOURS AFTER LOSING ⇒ the tool goes from RELIABLE to CONTESTED — per-plant orders on a court's clock. MATERIAL, NOT SYSTEMIC** · ⛔ **lifts only via SCOTUS, Congress, a real declared emergency, or a different statute — a slow list, hence durable; but ONE circuit and §202(c) specifically, and nothing shuts today** · ⛔ **THE OTHER SIDE OF THE SAME RULING: if forced retention fails, the answer is ACCELERATED PERMITTING — bullish GEV/ETN/PWR and the long-run DISCONFIRMER to scarcity pricing itself. Beyond 120 days ⇒ no grade move (16c(c)), but it is the mechanism to watch** · 🚩 **⬜ NRG IS A TIER-1 MERCHANT IPP IN THIS VERY NOTE WITH NO ROW IN `grades.md` — cheapest open hole, thesis already written (PSEG too).**

## 2026-09-15 ~9:00am PDT — ⭐⭐⭐⭐⭐ **EPA SIGNS THE GHG REPEAL — THE `:L358` REGISTERED MECHANISM FIRES WITHIN 72 HOURS OF THE §202(c) NARROWING; AND IT SPLITS THE BASKET, NOT LIFTS IT.** *(Zeldin signed 9/14, released 9/15; Jake's primary-confirmed relay + web verification; settled per rule 22.)*
### DATA (observed)
- **FINAL (signed): Partial Repeal of the 2024 Carbon Pollution Standards for fossil-fuel EGUs.** **PROPOSED (not final): rescission of the GHG significance finding for power plants under CAA §111** — which, if finalized, would bar a FUTURE EPA from re-regulating without redoing the endangerment groundwork. **Two rungs — the repeal is real now; the rescission is a proposal.**
- **EPA's own estimate: >$300B compliance-cost savings** *(⚠️ class 8 — the agency grading its own deregulation, multi-decade perimeter; filed as a claim, never a number).* **NRDC promises litigation.** State/local permitting, other air rules and interconnection all still apply.
- **Bernstein (Ocalan): GEV's order base is predominantly UTILITY-driven, not data-center-dependent; GEV +1-2% today after Monday's fall.**
### THESIS (interpretation — NOT fact)
- **★★★★★ THIS IS THE `:L358` LINE FIRING: "if forced retention fails, the answer is ACCELERATED PERMITTING."** The administration lost the LEGAL retention tool (D.C. Circuit narrowing §202(c), 9/12) and answered in 72h with an ECONOMIC one — coal needs no emergency order if the compliance cost retiring it vanishes. **Extends, does not supersede, the 9/12 entry: the coal-reversal disconfirmer's legal-economic leg is partially REPAIRED, so scarcity is marginally LESS durable than the 9/12 reading.**
- **★★★★ THE BASKET SPLITS: (a) BUILDOUT (GEV, VRT, ETN, PWR): tailwind real but on the WRONG AXIS** — binding constraints on new gas are turbines (GEV book sold out years), transformers, interconnection, state/local permitting — all untouched; the repeal clears only the COMPLIANCE axis (no CCS on new baseload gas). More demand into a sold-out book = pricing power, not near-term volume. **(b) SCARCITY (VST/CEG/TLN — and nuclear premia): the same mechanism CUTS AGAINST the premium** — deferred coal retirements + cheaper-to-run thermal = more competing dispatchable MW. Their own fleets save compliance cost (operational +), while the scarcity premium erodes (valuation −). **The 8:50 scan reached the same mechanism for nuclear ("loses a little of the scarcity value") without applying it to the IPPs — applied here to both.**
- **⚠️ THE TAPE COLLISION: these names fell −5 to −9% Monday on RATES with this policy already signed, and the 10Y is at 5.00 today — the discount-rate channel currently dominates the policy tailwind.**
### GRADES (16c) → [[grades]]
**NO MOVES — 16c(c): the payoff mechanism (IRP cycles, order-to-revenue, litigation clock) sits beyond 120 days, exactly as `:L358` already ruled; inside 120 days the rule is in court.** *(Jake's scan graded the ITEM 5/5 for the thesis environment — agreed at that altitude; the 120-day grade driver it is not — debated and held.)*
**📌 REGISTERED:** ⬜ the significance-finding rescission's finalization · ⬜ litigation venue/stay posture · ⬜ first utility IRP citing the repeal · Bernstein's utility-driven claim as GEV's non-AI demand floor.
**Links:** [[buildout-bottleneck-map]] · [[transmission-chain]] (stage: policy → capex commitments) · [[nuclear]] · `:L338` (the §202(c) entry this extends)

## 2026-09-16 ~6:55pm PDT — ⭐⭐⭐⭐⭐ **AEMA LAUNCHES: THE 8/7 CO-LOCATION THESIS GOES INSTITUTIONAL — GOOGLE/NVIDIA/EMERALD AI FORMALIZE FLEXIBLE-LOAD + COLOCATED GENERATION AS THE INDUSTRY ARCHITECTURE, WITH THE RATEPAYER WEDGE NOW POLLED AT ~80% BIPARTISAN.** *(9/16 open scan, approved; settled per rule 22.)*
### DATA (observed)
- **The AI Energy Management Alliance launched 9/16: founders Google, Nvidia, Emerald AI; participants include Anthropic, AES, National Grid, Constellation, NRG, RWE.** Stated objective: speed-to-power via FLEXIBLE data centers — **dynamically reduce grid draw using colocated generation, storage and workload management; conventional interconnection cited at 5-7+ years.**
- **The wedge, quantified: AP-NORC — 84% concerned data centers raise local electricity prices; Axios — 79% of Democrats and 76% of Republicans support making developers pay for their own grid upgrades.**
### THESIS (interpretation — NOT fact)
- **★★★★★ THE VAULT FILED THIS CONFIGURATION 8/7 (nuclear:L712): "behind-the-meter is the only generation class not hostage to grid politics — the one configuration both coalitions permit." Today the industry incorporated around it.** The realistic architecture is grid + colocated/BTM + storage + controllable load (the scan's refinement, adopted) — not full grid secession.
- **★★★ SECOND SCARCITY-EROSION VECTOR THIS WEEK: flexible/curtailable DC load peak-shaves exactly the tight-hours pricing the merchant IPPs monetize** — same direction as the EPA split (equipment/buildout > scarcity premium). Beneficiaries: BE, VRT, GEV, ETN, PWR + BTM generation; the scan's equipment-over-borrowers relative trade matches the filed rates read.
- **🚩 NRG IS IN THE ALLIANCE — the grades ledger's named "cheapest open hole" (Tier-1 merchant IPP, no row) gains its second datum in a week. ⬜ the workup is now overdue.**
**📌 REGISTERED:** ⬜ AEMA's technical framework doc (what "flexible" commits members to — curtailment depth/hours) · ⬜ NRG row · first utility rate case citing AEMA architecture.
**Links:** [[nuclear]] (:L712 the wedge) · [[transmission-chain]] (:L309) · [[buildout-bottleneck-map]] · [[grades]]

## 2026-09-19 ~12:35pm PDT (covers Thu-Fri; settled per rule 22) — ⭐⭐⭐⭐⭐ **THE WEDGE WEEK COMPLETES: AEMA (9/16 am) → RATEPAYER PROTECTION ACT 417-3 (9/16 pm) → AMAZON-GENERAC $8B BACKUP-POWER (9/16 filing) — POLLING, PRIVATE ARCHITECTURE, NATIONAL LEGISLATION AND HARD DOLLARS IN 48 HOURS.**
### DATA (observed)
- **The House passed the Ratepayer Protection Act 417-3: directs state regulators to CONSIDER standards making large-load customers bear their incremental generation/transmission costs.** No moratorium, no rate-setting — the margin is the message.
- **AMZN-Generac (Generac 8-K 9/16): long-term backup-generator supply; ~$2.4B initial deliveries 2027-28; warrants for up to 1.694M GNRC shares at $200.93, vesting on cumulative purchases up to $8B. GNRC ~+30%.** ⚠️ Explicitly BACKUP generators — resilience spend, not grid replacement.
- **Bernstein: procurement shifting to PREFABRICATED INTEGRATED MODULES — Schneider prefab capacity +270% in 2yrs; VRT OneCore scales 12.5MW modules → 1GW campuses; VRT/Schneider/ETN named strongest.**
### THESIS (interpretation — NOT fact)
- **★★★★★ THE SEQUENCE IS THE FINDING: polling (84%/~77% bipartisan) → the industry incorporates around the wedge (AEMA) → Congress votes 417-3 → a hyperscaler signs $8B of self-supplied resilience — all inside 48 hours. Every increment of internalized grid cost improves BTM/self-funded/colocated RELATIVE economics; the bill and AEMA are the same force from opposite directions.**
- **★★ The warrant structure is the vendor-equity extraction pattern rerun (cf. the WULF/Google penny warrants): the hyperscaler takes equity upside in the supplier as it hands it demand.** Modular prefab = a bottleneck-relief vector that concentrates margin in integrated OEMs — the equipment-over-borrowers trade again.
**📌 REGISTERED:** ⬜ GNRC roster initialization · ⬜ AEMA technical framework (curtailment depth/hours) · ⬜ first rate case citing the Act · NRG workup (third datum this week).
**Links:** [[nuclear]] (:L712 the wedge) · [[buildout-bottleneck-map]] · [[transmission-chain]] · [[grades]]

## 2026-09-20 ~7:45pm PDT — ⭐⭐⭐⭐⭐ **THE RE-CONCENTRATION READ (settled per rule 22): THE BUILDOUT DOESN'T DIE — IT FALLS BACK ONTO FIVE BALANCE SHEETS. AND BTM SURVIVES BY SELECTING FOR PERMIT AVAILABILITY.**
### THESIS (interpretation — Jake's synthesis + analysis)
- **★★★★★ THE 2027 $1.0-1.2T CONSENSUS CAPEX SPLITS INTO TWO FUNDING WORLDS: hyperscaler self-funded (IG at 78bp, >$200B YTD, clearing fine) vs the PERIPHERY (Jupiter 89-91 · CRWV 9.1% · Holtec pulled · SB Energy's 25-35× bridge) — the layer that was supposed to carry an INCREASING share. First-order consequence: not less capex — THE PERIPHERY'S SHARE COLLAPSES BACK ONTO THE STRONGEST BALANCE SHEETS; independents starve; distressed assets get ABSORBED (the 2007 pattern: the strong buy the hung projects). The industry CONCENTRATES — the opposite of what the developer ecosystem promised. Cap on absorption: shareholder tolerance, arriving exactly as the BCA depreciation cliff ($255B→$581B by '29) makes the P&L cost visible ⇒ THE FINANCING WALL AND THE DEPRECIATION WALL CONVERGE 2027-28 — the fulcrum's decision window.**
- **★★★★ BTM SELECTS BY PERMIT: the ledger's BTM failures are GAS failures on AIR PERMITS (VoltaGrid WA/GA, Synapse, Kalina, Clearway) — and WA state re-enacting gas-reduction building codes (Jake's catch) says the political climate opposes combustion at every scale. The surviving BTM architecture: fuel cells (different permit class; Jupiter shows the process still fights) · storage+flexible-load (AEMA — winning BY DEFAULT, no air permit at all) · and BTM NUCLEAR — the 8/7 wedge thesis strengthens a THIRD time (hostage to neither grid politics nor air-quality process), gated only by its clock.**
- **★★ The barbell restated: every permit denial and financing repricing raises $/MW on NEW supply and the scarcity value of ENERGIZED MW — the re-underwriting is bearish promised capacity, mechanically bullish operating capacity.**
**Links:** [[buildout-bottleneck-map]] (the ledger) · [[ai-financing-fragility]] · [[nuclear]] (:L712) · [[metered-compute]] · [[transmission-chain]]
- *(2026-09-24, Y'd)* **TEXAS DATA-CENTER PERMIT FREEZE (9/21) → the ERCOT LOAD-GROWTH leg of the VST/CEG (Calpine's Texas fleet) bull case weakens** — existing load untouched; the long-dated demand story and on-site-power orders (GEV/BE/CAT) slip until TCEQ resumes (update Oct 19; election Nov 3). Pay-your-own-way conditions (all infrastructure costs; LOWER residential bills) also compress data-center economics in Texas. → [[buildout-bottleneck-map]] 9/24.
